    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                # IMAGE
                image_abs_path = os.path.join(self.data_root, sample["image"].replace('./', ''))
                # image = sitk.GetArrayFromImage(sitk.ReadImage(image_abs_path))
                ...
                answer = "Answer: [Chest]: {} [Abdomen]: {} [Pelvis]: {}".format(sample["findings_chest"], sample["findings_abdomen"], sample["findings_pelvis"])
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(max_attempts):
            try:
                ...
                # TEXT
                one_question = random.choice(ast.literal_eval(sample["qa"])) # 랜덤하게 1개 뽑기.
                choices = "Choices: A. {} B. {} C. {} D. {}".format(one_question["options"]['A'], one_question["options"]['B'], one_question["options"]['C'], one_question["options"]['D'])
                question = one_question['question'] + ' ' + choices
                answer = "{}.".format(one_question["answer"])
