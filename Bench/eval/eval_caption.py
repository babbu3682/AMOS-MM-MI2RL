...
import evaluate
from green_metric import calculate_mean_green_score
...
def main():
    ...
    with open(output_path, mode='w') as outfile:
        ...
        writer.writerow(["Question", "Ground Truth", "Pred", "bleu", "rouge1", "meteor", "bert_f1", "green"])
        for sample in tqdm(test_dataloader):
            ...
            green_score = calculate_mean_green_score(pred_report=decoded_preds, gt_report=decoded_labels)
            result["green"] = green_score

            writer.writerow([question[0], answer[0], generated_texts[0], result["bleu"], result["rouge1"], result["meteor"], result["bert_f1"], result["green"]])
