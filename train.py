...
from LaMed.src.dataset.multi_dataset import TextDatasets, Text_With_ErrorNote_Datasets, CapDataset, Cap_Review_Dataset
...
class ModelArguments:
    ...
    model_type: Optional[str] = field(default=None, metadata={"help": "llama2, llama3"})
...
@dataclass
class DataArguments:
    data_root: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM", metadata={"help": "Root directory for all data."})

    # 0002
    # Caption data
    cap_data_train_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/train_cap_final.csv", metadata={"help": "Path to caption data."})
    cap_data_val_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/valid_cap_final.csv", metadata={"help": "Path to caption data."})
    
    # VQA data
    vqa_data_train_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/train_vqa_final.csv", metadata={"help": "Path to training VQA data."})
    vqa_data_val_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/valid_vqa_final.csv", metadata={"help": "Path to validation VQA data."})
    
    # Error Cap
    cap_errornote_data_train_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/cap_errornote_from_valid_0001.csv", metadata={"help": "Path to caption data."})
    cap_errornote_data_val_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/cap_errornote_from_valid_0001.csv", metadata={"help": "Path to caption data."})
    
    # Error VQA
    vqa_errornote_data_train_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/vqa_errornote_from_valid_0001.csv", metadata={"help": "Path to caption data."})
    vqa_errornote_data_val_path: str = field(default="/workspace/0.Challenge/MICCAI2024_AMOSMM/dataset/AMOS-MM/vqa_errornote_from_valid_0001.csv", metadata={"help": "Path to caption data."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    model_max_length: int = field(default=768, metadata={"help": "Maximum sequence length. Sequences will be right padded (and possibly truncated)."})  # old는 512였음. // 768
    ...
    save_steps: float = 0.04 # int = 2000 
    save_total_limit: int = 2 # 저장 최대 갯수, None으로 하면 제한없이 무한 저장.
    ...
    error_note: bool = False
    metric_for_best_model: str = "accuracy" # 추가
    greater_is_better: bool = True # 추가
    load_best_model_at_end: bool = True # 추가, 학습 종료 후 가장 좋은 모델을 불러옴, 설정 안하면 자동 "loss"로 설정...
...
class DataCollator:
    def __call__(self, batch: list) -> dict:
        ...
        images, input_ids, labels, attention_mask = tuple([b[key] for b in batch] for key in ('image', 'input_id', 'label', 'attention_mask'))
...
def main():
    ...
    if model_args.vision_tower is not None:
        if 'llama' in model_args.model_type:
            model = LamedLlamaForCausalLM.from_pretrained(model_args.model_name_or_path, cache_dir=training_args.cache_dir)
        else:
            raise ValueError(f"Unknown Model Type {model_args.model_type}")
    else:
        model = LlamaForCausalLM.from_pretrained(model_args.model_name_or_path, cache_dir=training_args.cache_dir)
    ...
    if training_args.lora_enable:
        ...
        for n, p in model.named_parameters():
            if any([x in n for x in ['vision_tower', 'mm_projector', 'embed_tokens', 'lm_head']]):
                p.requires_grad = True
    ...
    data_args.proj_out_num = model.get_model().mm_projector.proj_out_num  # 256
    rank0_print("vision tokens output from projector: ", data_args.proj_out_num)

    if training_args.error_note:
        train_dataset = Text_With_ErrorNote_Datasets(data_args, tokenizer, mode='train')
        # eval_dataset  = Text_With_ErrorNote_Datasets(data_args, tokenizer, mode='valid')
        eval_dataset  = Cap_Review_Dataset(data_args, tokenizer, mode='valid')

    else:
        train_dataset = TextDatasets(data_args, tokenizer, mode='train')
        # eval_dataset  = TextDatasets(data_args, tokenizer, mode='valid')
        eval_dataset = CapDataset(data_args, tokenizer, mode='valid')
    ...
    if model_args.resume is not None:
        trainer.train(resume_from_checkpoint=model_args.resume)
    else:
        trainer.train()
    ...
    if training_args.lora_enable:
        torch.save(model.state_dict(), os.path.join(training_args.output_dir, 'model_with_lora.bin'))
    else:
